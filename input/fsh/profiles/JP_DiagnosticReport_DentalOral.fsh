...
* insert SetDefinition(effective[x],  臨床的に関連する時刻または時間  【JP Core仕様】レポート作成日時)
* effective[x] ^comment = "観測値が関連する時間または期間。レポートの対象が患者である場合、これは通常、読影開始の時間であり、日付／時刻自体のみ提供される。実施日時。"
* insert SetDefinition(issued, このバージョンが作成された日時  【JP Core仕様】最新のレポート確定日時)
* performer only Reference(JP_Practitioner or JP_PractitionerRole or JP_Organization or CareTeam)
* insert SetDefinition(performer, レポート内容に責任をもつ診断的サービス  【JP Core仕様】レポート確定者)
* performer ^comment = "例：歯科医師など"

* insert SetDefinition(resultsInterpreter,  結果の一次解釈者  【JP Core仕様】レポートの作成者)
* resultsInterpreter ^comment = "例：歯科医師、歯科研修医など"

* insert SetDefinition(specimen, 診断レポートのもとになった検体に関する情報  【JP Core仕様】未使用)

* result only Reference(JP_Observation_DentalOral_ToothExistence)
* insert SetDefinition(result, 診断レポートの一部となるObservationsに関する情報  【JP Core仕様】検査結果)
* result ^comment = "Observationsは階層構造を持てる。"

* insert SetDefinition(media, 診断レポートに関連づけられたメディアに関する情報  【JP Core仕様】このレポートに関連づけられたキー画像)
* media.link ^comment = "例：口腔内写真、jpgで保存されている画像、3Dデータなど"

* insert SetDefinition(conclusion, 簡潔かつ臨床的な文脈で表現した診断レポートの要約結論（解釈、インプレッション） 【JP Core仕様】原則、未使用。レセプト傷病名などテキスト型で記載も可能。)
* insert SetDefinition(conclusionCode, 診断レポートの要約結論（解釈、インプレッション）を表すコード  【JP Core仕様】原則、未使用。レセプト傷病名などのCodeableConcept型で定義も可能。)