...
* extension contains $encounter-associatedEncounter|4.3.0 named associatedEncounter ..1
...
* class ^comment = "コードは列挙やコードリストの中で非常にカジュアルに定義されることもあれば、SNOMED CTのような非常にフォーマルな定義まである。 - 詳細は[HL7 v3コア・プリンシプル](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=481)を参照すること。"
...
入院｜外来｜外来｜救急＋。"
* classHistory.class ^comment = "コードは列挙やコードリストの中で非常にカジュアルに定義されることもあれば、SNOMED CTのような非常にフォーマルな定義まである- 詳細は[HL7 v3コア・プリンシプル](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=481)を参照すること。"
...
* reasonReference only Reference(JP_Condition or JP_Condition_Diagnosis or JP_Procedure or JP_Observation_Common or ImmunizationRecommendation)
* diagnosis.condition only Reference(JP_Condition or JP_Condition_Diagnosis or JP_Procedure)
