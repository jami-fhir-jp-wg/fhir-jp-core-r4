...
* identifier ^short = "Identifier(s) by which this encounter is known / このEncounterが知られている識別子（複数可）【詳細参照】"
...
* status ^isModifierReason = "This element is labeled as a modifier because it is a status element that contains status entered-in-error which means that the resource should not be treated as valid / この要素は、リソースを有効なものとして扱うべきではないことを意味する status entered-in-error を含む status 要素であるため、修飾子としてラベル付けされる。"
* statusHistory ^short = "List of past encounter statuses / 過去のEncounterのステータス一覧【詳細参照】"
...
* class ^short = "Classification of patient encounter / 患者とのEncounterの分類【詳細参照】"
...
* classHistory ^short = "List of past encounter classes / 過去のEncounter classのリスト【詳細参照】"
...
* type ^short = "Specific type of encounter / Encounterの具体的なタイプ【詳細参照】"
...
* type ^comment = "Since there are many ways to further classify encounters, this element is ..*.\r\n\r\nEncounterをさらに分類する方法はいろいろあるので、この要素は...*である。"
* serviceType ^short = "Specific type of service / 具体的なサービスの種類【詳細参照】"
...
* priority ^short = "Indicates the urgency of the encounter / このEncounterの緊急性を示す【詳細参照】"
...
* subject ^short = "The patient or group present at the encounter / このEncounterに立ち会う患者またはグループ【詳細参照】"
...
* episodeOfCare ^short = "Episode(s) of care that this encounter should be recorded against / このEncounterが記録されるべきケアのエピソード【詳細参照】"
...
* basedOn ^short = "The ServiceRequest that initiated this encounter / このEncounterを開始したServiceRequest【詳細参照】"
...
* participant ^short = "List of participants involved in the encounter / このEncounterに関わった参加者一覧【詳細参照】"
* participant ^definition = "The list of people responsible for providing the service.\r\n\r\nサービスを提供する責任がある人々のリスト。"
* participant.type ^short = "Role of participant in encounter / このEncounterにおける参加者の役割"
...
* participant.individual only Reference(JP_Practitioner or JP_PractitionerRole or RelatedPerson)
* appointment ^short = "The appointment that scheduled this encounter / このEncounterをスケジュールした予定【詳細参照】"
...
* period ^short = "The start and end time of the encounter / このEncounterの開始時間と終了時間【詳細参照】"
...
* length ^short = "Quantity of time the encounter lasted (less time absent) / Encounterが続いた時間の量（欠席時間が少ない）【詳細参照】"
...
* reasonCode ^short = "Coded reason the encounter takes place / Encounterの発生をコード化された理由の情報【詳細参照】"
...
* diagnosis.condition only Reference(JP_Condition or JP_Procedure)
* account ^short = "The set of accounts that may be used for billing for this Encounter / あるEncounterの請求に使用される会計セット【詳細参照】"
...
* hospitalization.reAdmission ^short = "The type of hospital re-admission that has occurred (if any). If the value is absent, then this is not identified as a readmission / 発生した病院の再入院のタイプ（もしあれば）、値がない場合これは再入院として識別されない"
...
* hospitalization.specialArrangement ^short = "Wheelchair, translator, stretcher, etc. / 車椅子、トランスレータ、ストレッチャなど"
...
* location ^short = "List of locations where the patient has been / 患者がいたことのある場所の一覧【詳細参照】"
...
* location.physicalType ^short = "The physical type of the location (usually the level in the location hierarchy - bed room ward etc.) / 場所の物理的なタイプ（通常は場所の階層のレベル - ベッド・病室・病棟など）"
...
* serviceProvider ^short = "The organization (facility) responsible for this encounter / このEncounterに責任を持つ組織（施設）【詳細参照】"
* serviceProvider ^definition = "The organization that is primarily responsible for this Encounter's services. This MAY be the same as the organization on the Patient record, however it could be different, such as if the actor performing the services was from an external organization (which may be billed separately) for an external consultation.  Refer to the example bundle showing an abbreviated set of Encounters for a colonoscopy.\r\n\r\nこのEncounterのサービスに主に責任を持つ組織。これは、患者記録上の組織と同じであってもよいが、例えば、サービスを実行しているアクターが外部組織のものである場合（外部コンサルテーションのために別個に請求される場合がある）など、異なる可能性がある。 大腸内視鏡検査のためのEncounterの省略されたセットを示すバンドル例を参照のこと。"
...
* partOf ^short = "Another Encounter this encounter is part of / このEncounterが属するもう一つのEncounter【詳細参照】"
