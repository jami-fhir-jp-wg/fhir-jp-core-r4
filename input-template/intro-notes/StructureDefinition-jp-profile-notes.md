...
2. birthdate, name 検索パラメータを使用して、Patientの検索をサポートすることが望ましい（**SHOULD**）。name検索パラメータは、HumanNameの文字列フィールド（family、give、prefix、suffix、および/またはtextを含む）のいずれかに一致するPatientリソースを検索する
   ...
   ```
   GET [base]/{Profile}?birthdate=eq{date}&name={string}
   ```

   例：

   ```
   GET [base]/{Profile}?birthdate=eq2000-10-10&name=山田%20太郎
   ```
...
3. birthdate, gender 検索パラメータを使用して、Patientの検索をサポートすることが望ましい（**SHOULD**）
   ...
   ```
   GET [base]/{Profile}?birthdate=eq{date}&gender={code}
   ```

   例：

   ```
   GET [base]/{Profile}?birthdate=eq2000-10-10&gender=male
   ```
...
4. birthdate, name, gender 検索パラメータを使用してPatientの検索をサポートすることが望ましい（**SHOULD**）、name検索パラメータはHumanNameの文字列フィールド（family、give、prefix、suffix、および/またはtextを含む）のいずれかに一致するPatientリソースを検索する
   ...
   ```
   GET [base]/{Profile}?birthdate=eq{date}&name={string}&gender={code}
   ```

   例：

   ```
   GET [base]/{Profile}?birthdate=eq2000-10-10&name=山田%20太郎&gender=male
   ```

5. name, phone 検索パラメータを使用して{Profile}の検索をサポートすることが望ましい（**SHOULD**）、name検索パラメータはHumanNameの文字列フィールド（family、give、prefix、suffix、および/またはtextを含む）のいずれかに一致する{Profile}リソースを検索する

   ```
   GET [base]/{Profile}?name={string}&phone={token}
   ```

   例：

   ```
   GET [base]/{Profile}?name=山田%20太郎&phone=0123456789
   ```

6. name, address-postalcode 検索パラメータを使用して{Profile}の検索をサポートすることが望ましい（**SHOULD**）、name検索パラメータはHumanNameの文字列フィールド（family、give、prefix、suffix、および/またはtextを含む）のいずれかに一致する{Profile}リソースを検索する、address-postalcodeは{Profile}.address.postalCode要素に対する検索パラメータであり日本の住所表記における郵便番号をキーにして検索することを想定している

   ```
   GET [base]/{Profile}?name={string}&address-postalcode={string}
   ```

   例：

   ```
   GET [base]/{Profile}?name=山田%20太郎&address-postalcode=1234567
   ```
...
1. 複合条件として、family,given,birthdate,gender,phone,address-postalcodename の各検索パラメータを複数指定したPatientの検索をサポートすることができる（MAY）
   ...
   ```
   GET [base]/{Profile}?family={string}&given={string}&birthdate=eq{date}&gender={token}&phone={token}&address-postalcode={string}
   ```

   例：

   ```
   GET [base]/{Profile}?family=山田&given=太郎&birthdate=eq2013-01-14&gender=male&phone=111-222-3333&address-postalcode=1234567
   ```
...
##### 追加検索パラメータ 
  ...
  カナ名称ソート用の検索パラメータ[jp-{Profile}-xxxx-sp]を追加している。
...
- $everything：[base]/{Profile}/$everything , [base]/{Profile}/[id]/$everything

  - この操作が呼び出された場合、すべての患者の、あるいは指定された特定の[id]をもつ{Profile}に関連した全てのリソースを返す


##### $everything 操作

この操作は、この操作が呼び出された特定の{Profile}リソースに関連する全ての情報を返す。応答は "searchset" タイプのBundleリソースである。サーバは、少なくとも、識別された医療従事者の{Profile}コンパートメントに含まれる全てのリソースと、それらから参照されるすべてのリソースを返すことが望ましい。

この操作の公式なURLは以下である。

```
http://hl7.jp/fhir/OperationDefinition/{Profile}-everything
```

```
URL: [base]/{Profile}/[id]/$everything
```
...
リクエスト：単一の{Profile}に関連する全てのリソースを取得する。

```
GET [base]/{Profile}/p001/$everything
[some headers]
```

レスポンス：指定された{Profile}に関連する全てのリソースを返す。
...
```json
    {
      ...
      "entry": [
        {
          "fullUrl": "http://example.org/fhir/{Profile}/p001",
          "resource": {
